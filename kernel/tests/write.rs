...
async fn test_commit_info() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (table, engine, store, table_name) in setup_test_tables(schema, &[], None, "test_table").await? {
...
async fn test_empty_commit() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (table, engine, _store, _table_name) in setup_test_tables(schema, &[], None, "test_table").await? {
        assert!(matches!(
...
async fn test_append() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (table, engine, store, table_name) in setup_test_tables(schema.clone(), &[], None, "test_table").await? {
        let commit_info = new_commit_info()?;
...
async fn test_append_invalid_schema() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (table, engine, _store, _table_name) in setup_test_tables(table_schema, &[], None, "test_table").await? {
        let commit_info = new_commit_info()?;
